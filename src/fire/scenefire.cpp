...
void SceneFire::initScene()
{
    ...
    const char * texName = "../../src/media/texture/fire.bmp";
...
void SceneFire::initBuffers()
{
    ...
    for( int i = 0; i < nParticles * 3; i += 3 ) {
        data[i] = glm::mix(-1.0f, 1.0f, randFloat());
        data[i+1] = 0.0f;
        data[i+2] = 0.0f;
    }
    glBindBuffer(GL_ARRAY_BUFFER, posBuf[0]);
...
{
	try {
		prog.compileShader("../../src/fire/shader/fire.vs",GLSLShader::VERTEX);
		prog.compileShader("../../src/fire/shader/fire.fs",GLSLShader::FRAGMENT);
